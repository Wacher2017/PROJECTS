...
// 标题
const name = defaultSettings.title || 'LMS 后台管理系统';
...
module.exports = {
    ...
    // 例如 https://www.lms.vip/。如果应用被部署在一个子路径上，你就需要用这个选项指定这个子路径。例如，如果你的应用被部署在 https://www.lms.vip/admin/，则设置 baseUrl 为 /admin/。
        ...
        config.when(process.env.NODE_ENV !== 'development', config => {
            config.plugin('ScriptExtHtmlWebpackPlugin')
                ...
                .use('script-ext-html-webpack-plugin', [{
                    // `runtime` must same as runtimeChunk name. default is `runtime` 正则匹配runtime文件名
                    inline: /runtime\..*\.js$/
                }]).end();
            config.optimization(
                {
